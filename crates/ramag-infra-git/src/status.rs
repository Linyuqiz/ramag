//! 工作区状态 + 分支。HEAD 走 gix；文件变更走 `git status --porcelain=v2 -z`；
//! 进行中操作看 .git/ 标记文件；ahead/behind 走 `git rev-list --left-right --count`
...
/// `git status --porcelain=v2 -z`：NUL 分记录，首字节标 entry type（1 普通 / 2 rename / ? 未跟踪 / u 冲突）
